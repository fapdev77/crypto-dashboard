...
import { OkxTxStats } from '../../../hooks/useOkxTransactions';
import { useFormatCurrency } from '../../../hooks/useFormatCurrency';
import { ChevronDown, ChevronUp } from 'lucide-react';
import { CoinIcon } from '../../ui/CoinIcon';

interface OkxTransactionNetChangeReportProps {
  filteredEntries: OkxTransactionLogEntry[];
  stats: OkxTxStats;
  tokenRates: Record<string, number>;
  isPrivateMode?: boolean;
}

export function OkxTransactionNetChangeReport({
  filteredEntries,
  stats,
  tokenRates,
  isPrivateMode = false,
}: OkxTransactionNetChangeReportProps) {
  ...
  const currencyReports = useMemo(() => {
    ...
    return Object.entries(grouped).map(([cur, curEntries]) => {
      ...
      const totalTrades = curEntries.filter(e => e.type.toUpperCase().includes('TRADE')).length;
      const buysCount = curEntries.filter(e => e.side.toLowerCase().includes('buy')).length;
      const sellsCount = curEntries.filter(e => e.side.toLowerCase().includes('sell')).length;
      ...
      const tradesLiquido = curEntries
        .filter(e => e.type.toUpperCase().includes('TRADE'))
