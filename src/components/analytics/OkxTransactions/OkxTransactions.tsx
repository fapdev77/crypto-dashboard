...
import { useOkxTransactions, OkxTxFilters } from '../../../hooks/useOkxTransactions';
...
import { OkxTransactionRow } from './OkxTransactionRow';
import { OkxTransactionFilters, OKX_TX_TYPES, okxTypeColorMap, okxTypeHexColorMap } from './OkxTransactionFilters';
import { OkxTransactionProgress } from './OkxTransactionProgress';
import { OkxTransactionDetailsModal, OkxDetailsModalType } from './OkxTransactionDetailsModal';
import { exportToCSV, exportToExcel, exportToPDF, ExportConfig } from '../../../utils/exportUtils';
import { LogManager } from '../../../services/LogManager';
import { CoinIcon } from '../../ui/CoinIcon';

export function OkxTransactions() {
  const [filters, setFilters] = useState<OkxTxFilters>({
    search: '',
    category: 'All',
    type: 'All',
    currency: 'All',
  ...
  const { entries, filteredEntries, isLoading, isSyncing, isCalculatingUsd, progress, error, stats, tokenRates } = useOkxTransactions(filters);
  const formatCurrency = useFormatCurrency();
  const { isPrivateMode } = usePrivacy();
  const [expandedId, setExpandedId] = useState<string | null>(null);
  const [exportMenuOpen, setExportMenuOpen] = useState(false);
  const [detailsModalType, setDetailsModalType] = useState<OkxDetailsModalType>(null);
  ...
  const handleExport = (format: 'csv' | 'excel' | 'pdf') => {
    ...
    const config: ExportConfig = {
      title: 'OKX Transactions Report',
      filename: `OKX_Transactions_${Date.now()}`,
      headers,
      rows,
    };
    ...
    LogManager.info('OkxTransactions', `Exported ${rows.length} rows as ${format}`);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Filters */}
      <OkxTransactionFilters filters={filters} setFilters={setFilters} availableCurrencies={availableCurrencies} />
            ...
            <div className="flex flex-col">
              ...
              <div className="flex flex-wrap gap-1 mt-2">
                {Object.entries(stats.typeBreakdown).slice(0, 4).map(([type, count]) => {
                  const typeLabel = OKX_TX_TYPES.find(t => t.value === type)?.label || type;
                  const typeClass = okxTypeColorMap[type] || 'text-[#8E9299] bg-[#2a2b30]/50 border-[#2a2b30]';
                  return (
                  ...
                  <PieChart>
                    <Tooltip
                      ...
                      formatter={(value, name) => {
                        const typeLabel = OKX_TX_TYPES.find(t => t.value === name)?.label || name;
      ...
      {!isLoading && filteredEntries.length === 0 && (
        <div className="flex flex-col items-center justify-center p-12 bg-[#151619] border border-[#2a2b30] rounded-xl">
          ...
          <p className="text-[#8E9299] text-center">
            {entriesTotal > 0 ? 'No transactions match the current filters.' : 'No OKX transactions found. Add OKX API keys to start syncing.'}
          </p>
        </div>
      )}
