...
import { useBitgetTransactions, BitgetTxFilters } from '../../../hooks/useBitgetTransactions';
...
import { BitgetTransactionRow } from './BitgetTransactionRow';
import { BitgetTransactionFilters, BITGET_TX_TYPES, bitgetTypeColorMap, bitgetTypeHexColorMap } from './BitgetTransactionFilters';
import { BitgetTransactionProgress } from './BitgetTransactionProgress';
import { BitgetTransactionDetailsModal, BitgetDetailsModalType } from './BitgetTransactionDetailsModal';
import { exportToCSV, exportToExcel, exportToPDF, ExportConfig } from '../../../utils/exportUtils';
import { LogManager } from '../../../services/LogManager';
import { CoinIcon } from '../../ui/CoinIcon';

export function BitgetTransactions() {
  const [filters, setFilters] = useState<BitgetTxFilters>({
    search: '',
    category: 'All',
    type: 'All',
    currency: 'All',
  ...
  const { entries, filteredEntries, isLoading, isSyncing, isCalculatingUsd, progress, error, stats, tokenRates } = useBitgetTransactions(filters);
  const formatCurrency = useFormatCurrency();
  const { isPrivateMode } = usePrivacy();
  const [expandedId, setExpandedId] = useState<string | null>(null);
  const [exportMenuOpen, setExportMenuOpen] = useState(false);
  const [detailsModalType, setDetailsModalType] = useState<BitgetDetailsModalType>(null);
  ...
  const filterMonitorKey = useMemo(() => JSON.stringify(filters), [filters]);
  ...
  const handleExport = (format: 'csv' | 'excel' | 'pdf') => {
    ...
    const headers = [
      ...
      'Amount', 'Fee', 'Cash Flow', 'Change', 'Balance',
      'Trade ID', 'Order ID'
    ];
    const rows: (string | number)[][] = filteredEntries.map(e => [
      ...
      e.cashFlow || '0',
      e.change || '0',
      e.cashBalance || e.balance || '0',
      e.tradeId || '',
      e.orderId || '',
    ]);

    const config: ExportConfig = {
      title: 'Bitget Transactions Report',
      filename: `Bitget_Transactions_${Date.now()}`,
      headers,
      rows,
    };
    ...
    LogManager.info('BitgetTransactions', `Exported ${rows.length} rows as ${format}`);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Filters */}
      <BitgetTransactionFilters filters={filters} setFilters={setFilters} availableCurrencies={availableCurrencies} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="flex flex-col">
              ...
              <div className="flex flex-wrap gap-1 mt-2">
                {Object.entries(stats.typeBreakdown).slice(0, 4).map(([type, count]) => {
                  const typeLabel = BITGET_TX_TYPES.find(t => t.value === type)?.label || type;
                  const typeClass = bitgetTypeColorMap[type] || 'text-[#8E9299] bg-[#2a2b30]/50 border-[#2a2b30]';
                  return (
                  ...
                  <PieChart>
                    <Tooltip
                      ...
                      formatter={(value, name) => {
                        const typeLabel = BITGET_TX_TYPES.find(t => t.value === name)?.label || name;
          ...
          <div className="bg-[#161b22] rounded-lg p-4 border border-[#2a2b30] flex flex-col justify-center">
            ...
            <span className={`text-xl font-bold ${
              new Big(stats.aggregatedUsd.totalFees).lt(0) 
                ? 'text-[#FF4444]' 
                : new Big(stats.aggregatedUsd.totalFees).gt(0) 
                ? 'text-[#00C853]' 
                : 'text-white'
            }`}>
              {(() => {
                const feeBig = new Big(stats.aggregatedUsd.totalFees);
                if (feeBig.eq(0)) return `${maskVal(0)} USD`;
                if (feeBig.lt(0)) {
                  // Negative value (fee paid) - maskVal outputs -$X.XX
                  return `${maskVal(feeBig.toString())} USD`;
                }
                // Positive value (fee rebate received)
                return `+${maskVal(feeBig.toString())} USD`;
              })()}
            </span>
            ...
            <div className="flex items-center gap-2 mt-1">
              <span className="text-[10px] text-[#8E9299]">
                {new Big(stats.aggregatedUsd.totalFees).gt(0) ? 'Trading fee rebates' : 'Trading fees paid'}
              </span>
              {isCalculatingUsd && (
                <AppTooltip description="Fetching real-time USD prices for non-stable assets...">
                  <span className="flex items-center gap-1 text-[10px] text-[#FF9C2E] ml-auto">
      ...
      {!isLoading && filteredEntries.length === 0 && (
        <div className="flex flex-col items-center justify-center p-12 bg-[#151619] border border-[#2a2b30] rounded-xl">
          ...
          <p className="text-[#8E9299] text-center">
            {entriesTotal > 0 ? 'No transactions match the current filters.' : 'No Bitget transactions found. Add Bitget API keys to start syncing.'}
          </p>
        </div>
      )}
      ...
      {filteredEntries.length > 0 && (
        <div className="flex-1 overflow-auto hide-scrollbar">
          ...
          {entriesTotal > 0 && (
            <div className="mt-4">
              <Pagination
                id="bitget-tx-pagination-bottom"
                currentPage={currentPage}
                totalItems={entriesTotal}
                itemsPerPage={50}
                onPageChange={setCurrentPage}
