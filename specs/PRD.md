...
- **History:** As a trader, I want to view my past closed positions/trades to analyze my historical performance.
...
- **Testing:** As a developer or user evaluating the platform, I want to toggle a Mock Data mode to populate the dashboard with realistic temporary metrics without linking real keys.

## 4. Key Workflows
1. **API Onboarding:** User adds keys through a modal. Keys are saved in `localStorage`.
2. **Connection Bootstrap:** App fetches the stored keys and initiates REST calls (via Proxy) to load initial states (e.g. Bybit Wallet Snapshots or user History) and directly opens WSS (Secure WebSockets) for real-time streams.
...
5. **Historical Reports:** Through the UI, users can query time-range bounded PnL history via dedicated REST endpoints supported by the local Proxy.
